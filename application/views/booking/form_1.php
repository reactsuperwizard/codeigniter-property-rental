...
foreach ($this->reply['config']['venues'] AS $v){
  ...
  $venueOptions[]='<option class="v'.$v['address_id'].'" data-is_venue="1" value="'.$v['address_id'].'">Venue: '.$v['name'].'</option>';
}
...
<div class="row padded"><div class="col-sm-12">
  ...
  <div class="form-group">
    <div class="col-sm-3 col-md-2">
      <div class="row">
        ...
        <div class="col-xs-12 newCustomer"><a class="btn btn-sm btn-warning pull-right" onclick="bookings.customer.choose();">Add</a></div>
      </div>
    </div>
    <div class="col-sm-9 col-md-10" id="bookingCustomer">
  ...
  <div class="form-group residentialAddress">
    ...
    <div class="col-sm-9 col-md-10">
      ...
      <div class="panel panel-default" style="margin:0px;">
          <div class="panel-body">
            <div><select class="form-control" name="residential_address_id" onchange="bookings.address.choose('residential');" data-chosen=""><option value=""><?php echo $this->lang->phrase('choose_one'); ?></option><optgroup label="current" class="userAddresses"></optgroup><option value="0"><?php echo $this->lang->phrase('add_new'); ?></option><option class="skip" value="skip"><?php echo $this->lang->phrase('skip'); ?></option></select></div>
  ...
  <div class="form-group deliveryAddress">
    ...
    <div class="col-sm-9 col-md-10" id="bookingDelivery">
        <div class="panel panel-default" title="residential address" style="margin:0px;">
          <div class="panel-body">
            <div class="deliveryAddressChoice"><select class="form-control" name="delivery_address_id" onchange="bookings.address.choose('delivery');" data-chosen=""><option value=""><?php echo $this->lang->phrase('choose_one'); ?></option><optgroup label="User addresses" class="userAddresses"></optgroup><optgroup label="Venues"><?php echo $venueMenu; ?></optgroup><optgroup label="Other"><option class="residential" value="residential"><?php echo $this->lang->phrase('residential'); ?></option><option value="0"><?php echo $this->lang->phrase('add_new'); ?></option><option class="skip" value="skip"><?php echo $this->lang->phrase('skip'); ?></option></optgroup></select></div>
              <div class="row">
...
<div id="bookingEntryAttachment" style="display:none;position: fixed;top:0px;left:0px;bottom:0px;right:0px;width:100%;height:100%;background-color:rgba(0,0,0,0.5);padding: 5%;">
  ...
  <div class="panel panel-default">
    ...
    <div class="col-sm-6">
      ...
      <a class="btn btn-warning" onclick="bookings.entries.attachItem(_NS.DOM.getFormData('#bookingItemForm'));">Add new</a></form>
      ...
      <input type="radio" name="discount_type" value="amount"/>&nbsp;$
      <input type="text" class="form-control" name="discount_value" value="0"/>
      <a class="btn btn-warning" onclick="bookings.entries.attachService(_NS.DOM.getFormData('#bookingServiceForm'));">Add new</a></form>
    </div>
  </div>
    </div>
  </div>
    ...
    _NS.post('<?php echo NS_BASE_URL; ?>booking/edit','booking_id='+ID,{
      'success':function(reply){
        ...
        for(contactField in reply.data['delivery_contact']){
          document.querySelector('#bookingForm input[name="delivery_address['+contactField+']"]').dataset['chosen']=reply.data['delivery_contact'][contactField];
        }
...
bookings.customer={
  'reset':function(){
    _NS.DOM.enable('#bookingCustomer input');
    document.querySelector('#bookingForm .newCustomer').style.display='block';
    document.querySelector('#bookingCustomer .customerDetails').style.display='none';
    document.querySelector('#bookingCustomer input[name="customer_id"]').value=0;
    document.querySelector('#bookingCustomer .choice').style.display='block';
    
    document.querySelectorAll('#bookingForm .residentialAddress, #bookingForm .deliveryAddress').forEach(function(a){
      a.style.display='none';
    });
    
    bookings.address.reset('residential');
    bookings.address.reset('delivery');
  }
  ,'choose':function(userData){
    ...
    if (typeof(userData)=='undefined'){
      document.querySelector('#bookingCustomer input[name="customer_id"]').value=0;
      _NS.DOM.resetFields('#bookingForm #bookingCustomer .customerDetails');
      _NS.DOM.enable('#bookingCustomer input');
      
      document.querySelector('#bookingForm .newCustomer').style.display='block';
      document.querySelector('#bookingCustomer .customerDetails').style.display='block';
      bookings.address.updateList('residential');
      bookings.address.updateList('delivery');
      return false;
    }
    document.querySelector('#bookingForm .newCustomer').style.display='none';
    _NS.DOM.disable('#bookingCustomer input');
    ...
    document.querySelector('#bookingCustomer .customerDetails').style.display='block';
    bookings.address.updateList('residential',userData['user_id']);
    bookings.address.updateList('delivery',userData['user_id']);
  }
  ,'unlock':function(){
    _NS.DOM.enable('#bookingCustomer input');
    document.querySelector('#bookingForm .newCustomer').style.display='block';
...
bookings['address']={
  'reset':function(mode){
    var mode=mode;
    document.querySelectorAll('#bookingForm select[name="'+mode+'_address_id"] .userAddresses').innerHTML='';
    _NS.DOM.resetFields('#bookingForm div.'+mode+'Address');
    document.querySelectorAll('#bookingForm div.'+mode+'Address .addressDetails').forEach(function(el){
  ...
  ,'choose':function(mode,chosenAddressID){
    ...
    if (typeof(chosenAddressID)=='undefined'){
      chosenAddressID=document.querySelector('#bookingForm select[name="'+mode+'_address_id"]').value;
    }
    else {
      document.querySelector('#bookingForm select[name="'+mode+'_address_id"]').value=chosenAddressID;
    }
    document.querySelectorAll('#bookingForm div.'+mode+'Address .addressDetails input').forEach(function(f){
    ...
    switch(chosenAddressID){
      ...
      case 'residential':
        ...
        document.querySelectorAll('#bookingForm div.'+mode+'Address .contactDetails').forEach(function(el){
          el.style.display='block';
        });
        
        _NS.DOM.enable('#bookingForm div.'+mode+'Address .addressDetails input');
      break;
      case '':
        document.querySelectorAll('#bookingForm div.'+mode+'Address .addressDetails').forEach(function(el){
          el.style.display='none';
      ...
      break;
      default:
        _NS.DOM.fillFields('#bookingForm div.'+mode+'Address',addressCache[chosenAddressID],mode+'_address');
        _NS.DOM.disable('#bookingForm div.'+mode+'Address input');
        document.querySelectorAll('#bookingForm div.'+mode+'Address .addressDetails').forEach(function(el){
          el.style.display='block';
        });
        if (document.querySelector('#bookingForm select[name="'+mode+'_address_id"] .v'+chosenAddressID).dataset['is_venue']=='0'){
          _NS.DOM.enable('#bookingForm div.'+mode+'Address .contactDetails input');  
        }
      break;
    }
    document.querySelector('#bookingForm select[name="'+mode+'_address_id"]').dataset['chosen']='';
  }
  ,'updateList':function(mode,userID){
    ...
    if (typeof(userID)!='undefined'){
      _NS.post('<?php echo NS_BASE_URL; ?>customer/addresses',{'user_id':userID,'address_type':mode},{'success':function(reply){
        ...
        document.querySelector('#bookingForm select[name="'+mode+'_address_id"] .userAddresses').innerHTML=result.join('');
        setTimeout(function(_this,mode){_this.choose(mode,document.querySelector('#bookingForm select[name="'+mode+'_address_id"]').dataset['chosen']);},100,_this,mode);
        
      }},1);
    }
    else {
      document.querySelector('#bookingForm select[name="'+mode+'_address_id"] .userAddresses').innerHTML='';
    }
  }
};
...
bookings['entries']={
  ...
  ,'addItem':function(entryType,data){
      ...
      _this.active[_this.total]={'type':'item','itemID':((entryType=='regular')?data['item_id']:0)};
...
.replace('_INITIAL_DISCOUNT_TYPE_',data['discount_type'])
  ...
  ,'updatePrice':function(entryID){
    ...
    discountValue=variationElement.querySelector('input[name="entry['+entryID+'][discount_value]"]').value;
    if (discountType!=variationElement.querySelector('input[name="entry['+entryID+'][previous_discount_type]"]').value){
      switch(discountType){
        case 'percentage':
  ...
  ,'recalculate':function(updateRequired){
   ...
   //console.log('Subtotal: '+subtotal+'; Discount: '+discountType+'('+discountValue+')');
    ...
    variationElement.querySelector('.totalItemMultipliedPrice').innerHTML=totals['item']['multipliedPrice'];
    variationElement.querySelector('.totalItemDiscount').innerHTML='-'+totals['item']['discount'];
    variationElement.querySelector('.totalItemPrice').innerHTML=totals['item']['multipliedPrice']-totals['item']['discount'];
    variationElement.querySelector('.totalServiceMultipliedPrice').innerHTML=totals['service']['multipliedPrice'];
    variationElement.querySelector('.totalServiceDiscount').innerHTML='-'+totals['service']['discount'];
    variationElement.querySelector('.totalServicePrice').innerHTML=totals['service']['multipliedPrice']-totals['service']['discount'];
    variationElement.querySelector('.subtotalPrice').innerHTML=subtotal;
...
bookings['logistics']={
  ...
  ,'render':function(mode,code){
    ...
    for (itemID in _this[mode][code]){
      if (_this[mode][code][itemID]==1){
        ...
        if (typeof(itemCache[itemID])!='undefined'){
          result.push('<span class="quantity'+itemID+'">'+_this.itemIDs[itemID]['quantity']+'</span> x '+itemCache[itemID]['title']+' <a onclick="bookings.logistics.prepareUpdate(\''+itemID+'\',\''+mode+'\');">change</a>');
        }
      }
    }
    if (result.length>0){
      if (container==null){
        document.querySelector('#bookingForm #bookingLogistics .'+mode+'Panel').insertAdjacentHTML('beforeend','<div class="'+mode+code+'"><label>'+code+'</label><ul></ul></div>');
      }
  ...
  ,'updateQuantity':function(itemID,quantity){
    var _this=this,itemID=itemID,quantity=quantity;
  ...
  ,'prepareUpdate':function(itemID,mode){
    ...
    container.querySelector('.itemTitle').innerHTML=itemCache[itemID]['title'];
    ...
    $('#bookingLogisticsUpdateForm input[name="date"]').datepicker('option',dateOppositeDirection,'today');
    $('#bookingLogisticsUpdateForm input[name="date"]').datepicker('option',dateDirection,new Date(dateParts[0],(dateParts[1]-1),dateParts[2]));
  }
