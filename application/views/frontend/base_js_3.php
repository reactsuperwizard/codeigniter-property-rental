  ...
  ,'items':{
    ...
    ,'init':function(){
      var _this=this,_that=NS_Rental, tags=[], items=[]
        , categories=[], categoryUL=[], labels=_that.config.labels[_that.config.languages.chosen];
      ...
      if (!_this.initiated){
        ...
        _that.config.categories['default'].forEach(function(c){
          ...
          if (c>0){
            categoryUL.push('<li onclick="NS_Rental.items.updateCategory('+c+');">'+_that.config.categories[c]['title']+'</li>');
          }
        ...
        _that.config.tags['default'].forEach(function(t){
          tags.push('<li class="tag'+t+'" onclick="NS_Rental.tags.toggle('+t+');">'+_that.config.tags[t]['code']+'</li>');
        });
    ...
    ,'updateCategory':function(categoryID){
      ...
      _this['elements']['chosenCategoryID']=(typeof(categoryID)!='undefined')?categoryID:0;
    ...
    ,'getPaginated':function(page){
      ...
      ,categoryID=this['elements']['chosenCategoryID'];
      this.chosen=0;
      ...
      finalItems=_this.config.categories[this.elements['chosenCategoryID']]['items'];
      //console.log('finalItems: '+document.querySelector('#NS_Rental > .header select[name="category_id"]').value+' / '+_this['items']['chosenCategoryID']);
      
      //finalItems=_this.config.categories[this.chosenCategoryID]['items'];
