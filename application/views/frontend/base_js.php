    ...
    ,'init':function(){
      ...
      if (_this.initiated===false){
        ...
        _that.config.items['default'].forEach(function(i){
          var i=i,packed=[],packedItem={},packedItemID=0;
          if (typeof(_that['config']['items'][i]['package'])!='undefined'){
            for(packedItemID in _that['config']['items'][i]['package']){
              packedItem=_that['config']['items'][packedItemID];
              packed.push(_that['config']['items'][i]['package'][packedItemID]+' x '+packedItem['title']+' of '+packedItem['quantity']);
            }
          }
          //if ()
          items.push('<div id="NS_Rental__item'+i+'" class="item'+i+' col-xs-4 col-sm-3 col-md-3 col-lg-3 '+((_that.config.items[i]['quantity']==0)?' bg-warning':'')+'" >'
            +'<div class="thumbnail">'
              ...
              +'</div>'
              +'<div class="caption">'
                +'<span class="title" onclick="NS_Rental.items.view('+i+');">'+_that.config.items[i]['title']+' ('+_that.config.items[i]['quantity']+')'+'</span>'
                +((packed.length>0)?('<br/>'+packed.join('<br/>')):'')
