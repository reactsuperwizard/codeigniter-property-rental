...
/**/
  ...
  function validateCustomerDetails($initialCustomerID=0){
    ...
    if (!empty($_POST['customer'])){
      $customerDetails=array_merge($customerDetails,$_POST['customer']);
    }
    
    if(empty($customerDetails)){
      $this->error('Customer should be set',false,true);
    }
    $resultFields=array('first_name','last_name','company_name','email');
    $result=array();
    ...
    if ($this->hasErrors()){
      $this->reply['data']['failed_section']='customer';
      $this->reply['data']['customer']=$result;
      $this->returnJSON();
    }
  ...
  function validateCustomerDetails___(){
    ...
    if (empty($_POST['customer']['first_name']) || empty($_POST['customer']['last_name'])){
      $this->error('Customer name should be set',false,true);
    }
    ...
    if (!preg_match("/^[a-zA-Z`']{2,}$/", $_POST['customer']['first_name'])){
      $this->error('Customer first name should be set',false,true);
    }
    if (!empty($_POST['customer']['last_name'])){
      if (!preg_match('/^[a-zA-Z`\']{2,}$/', $_POST['customer']['last_name'])){
        $this->error('Customer last name should be set',false,true);
      }
  ...
  function validateResidentialAddress(){
    ...
    $initialResidentialAddressID=(($initialResidentialAddressID>0)?$initialResidentialAddressID:$_POST['residential_address_id'])*1;
    
    $residentialAddressID=$this->Address_model->validate($initialResidentialAddressID,'residential',$requiredFields,(!empty($_POST['residential_address'])?array_merge(
  ...
  function validateAddresses($initialResidentialAddressID=0,$initialDeliveryAddressID=0,$quoteID=0){
    ...
    $initialResidentialAddressIDRequest=intval($_POST['residential_address_id']);
    $initialResidentialAddressID=($initialResidentialAddressIDRequest>0)?$initialResidentialAddressIDRequest:$initialResidentialAddressID;

    $initialDeliveryAddressIDRequest=intval($_POST['delivery_address_id']);
    $initialDeliveryAddressID=($initialDeliveryAddressIDRequest>0)?$initialResidentialAddressIDRequest:$initialDeliveryAddressID;
    $this->log('Initial residential ID: '.$initialResidentialAddressID);
    ...
    $this->log('Residential ID: '.$residentialAddressID);
    ...
    if ($residentialAddressID>0){
      ...
      if (
        !empty($_POST['residential_delivery']) || @$_POST['delivery_address_id']=='residential' 
        || $initialDeliveryAddressID==$initialResidentialAddressID
      ){
        $deliveryAddressID=$residentialAddressID;
      }
      else {
        $deliveryAddressID=$this->Address_model->validate($initialDeliveryAddressID,'delivery',$requiredFields,(!empty($_POST['delivery_address'])?array_merge(
