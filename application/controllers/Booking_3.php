...
/** /   
  ...
  function validateDeliveryAddress($initialDeliveryAddressID=0){
    ...
    if (!empty($_POST['residential_delivery'])){
      $this->deliveryAddressID=(empty($this->residentialAddressID))?$this->residentialAddressID:intval($_POST['residential_address_id']);
      return false;
    }
    ...
    $phone=preg_replace('/[^0-9]/','',$_POST['delivery_address']['contact_phone']);
    if (!preg_match('/^(0[0-9])?[0-9]{8}$/',$phone)){
      $this->error('Delivery phone should be valid','delivery_address[phone]');
    }
    $thid->deliveryContact=json_encode($deliveryContact);
/** /
  ...
  function __validateAddresses($initialResidentialAddressID=0,$initialDeliveryAddressID=0,$quoteID=0){
    $this->newAddresses=array();
    $this->load->model('Address_model');
  ...
  function acceptDelivery(){
    $this->validateDeliveryAddress();
    if (empty($_POST['delivery_address']['contact_phone'])){
      $this->error('Phone should be valid','delivery_address[contact_phone]');
    }
