  ...
  function filtered($config=array()){
    ...
    if (!empty($searchTerm)){
      $this->datatable->filter()->group_start()
        ->like('q.name',$searchTerm,'after')
        ->group_end();
    }
    
    $order='q.quote_id';
    $this->datatable
      ->order_by('q.quote_id DESC')
      ->run($this->reply['data']);
...
/**/
        ...
        if (!$this->hasErrors()){
          if ($this->quoteID==0){
            ...
            if ($dataSet['customer_id']==0){
              $this->db->insert('user',array_merge($this->customerDataSet,array('role_id'=>$this->db->get_where('role',array('code'=>'customer'),1)->row_array()['role_id'])));
              $dataSet['customer_id']=$this->db->insert_id();
            }
            else {
              $this->db->where('user_id',$dataSet['customer_id'])->update('user',$this->customerDataSet);
            }
  ...
  function loadDetails($quoteID=0,$sections=array()){
    ...
    if ($this->reply['data']['customer_id']>0){
      $this->reply['data']['customer']=$this->db
        ->select('user_id,first_name,last_name,email,company_name AS `company`,IFNULL(phone,\'\') AS phone')
    ...
    $this->reply['data']['variations']=$this->db->get_where('quote_variation',array('quote_id'=>$quoteID))->result_array();
  ...
  function validateCustomerDetails(){
    ...
    if (empty($_POST['customer']['first_name']) || empty($_POST['customer']['last_name'])){
      $this->error('Customer name should be set','customer[first_name]');
      $this->error('Customer name should be set','customer[first_name]');
    }
...
/** /
  ...
  function validateAddresses(){
    ...
    elseif ($_POST['delivery_address_id']!='skip'){ 
      if ($deliveryAddressID==0){
        $this->Address_model->save(array_merge($_POST['delivery_address'],array('type'=>'delivery')),true,array('line_1','city'));
        $this->newAddresses[]=$deliveryAddressID;
      }
    }
    ...
    if ($deliveryContact['contact_phone']){
