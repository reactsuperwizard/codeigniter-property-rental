...
class Item extends NS_Rental_Controller {
  function __construct($config = array()) {
    ...
    switch ($IP){
      ...
      $this->userInfo=array('user_id'=>1,'email'=>'admin@freelance.nws','role'=>'admin','name'=>'admin');
        //
      break;
      default:
        die();
  ...
  function filtered(){
    ...
    if (!empty($_POST['period'])){
      ...
      $startDate=preg_replace('[^0-9\-]','',$_POST['period']['start_date']);
      $startDateParts=explode('-',$startDate);
      if (!empty($_POST['period']['start_time'])){
        $startTime=preg_replace('[^0-9:]','',$_POST['period']['start_time']);
      }
      else {
        $startTime='14:00';
      }

      $endDate=preg_replace('[^0-9\-]','',$_POST['period']['end_date']);
      $endDateParts=explode('-',$endDate);
      if (!empty($_POST['period']['end_time'])){
        $endTime=preg_replace('[^0-9:]','',$_POST['period']['end_time']);
      }
      else {
        $endTime='10:00';
      }
      ...
      if ($startTimestamp<(MAIN_TIMESTAMP+MINIMAL_RENT_TIME_PREFIX)){
      ...
      if ($endTimestamp<$startTimestamp){
        return $this->error($this->lang->phrase('end_should_be_after_start'));
      }
      ...
      $startTimestamp=$startTimestamp-DELIVERY_TIME_PREFIX;
      $endTimestamp=$endTimestamp+COLLECTION_TIME_SUFFIX;
      $config['rentTimestamps']=array('start'=>$startTimestamp,'end'=>$endTimestamp);
      /**/
      ...
      if (!empty($_POST['unlock'])){
        ...
        $config['lockHash']=$this->ItemLock_model->unlock($_POST['unlock']['type'],($_POST['unlock']['id']*1),$startTimestamp,$endTimestamp);
      }
  ...
  function save(){
    ...
    if (!$this->hasErrors()){
      ...
      if (!$this->hasErrors()){
        ...
        $this->db->where('item_id',$itemID)->update('item',array('file_filter_id'=>$fileFilterID));
      }
  ...
  function loadDetails($itemID=0,$sections=array()){
    ...
    $this->reply['data']=$this->db->select('i.*,t1.content AS title,t2.content AS description')
      ->join('translation AS t1','t1.target_object_id=i.item_id AND t1.target_object_type_field_id='.ITEM_OBJECT_TITLE.' AND t1.language_id=1','inner')
      ->join('translation AS t2','t2.target_object_id=i.item_id AND t2.target_object_type_field_id='.ITEM_OBJECT_DESCRIPTION.' AND t2.language_id=1','left')
      ->join('file AS f','f.file_filter_id=i.file_filter_id AND f.file_filter_id>0 AND `order`=1','left')
